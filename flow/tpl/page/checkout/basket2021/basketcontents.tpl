...
<div class="basket-content-wrapper">
    <h2 class="basket-title">Товары в корзине</h2>
    ...
    <form name="basket[{$basketindex}]" id="basket_form" action="[{$oViewConf->getSelfActionLink()}]" method="post">
        ...
        <div class="visible-xs  d-block d-sm-none" id="basketcontents_list">
            [{include file="page/checkout/basket2021/basketcontents_list.tpl"}]
        </div>
        [{* Für alle Geräte >=768px sichtbar *}]
        <div class="hidden-xs d-none d-sm-block" id="basketcontents_table">
            [{include file="page/checkout/basket2021/basketcontents_table.tpl"}]
        </div>
    </form>
    ...
    [{block name="checkout_basket_vouchers"}]
        <div id="basketVoucher" class="pull-left basket-coupon">
            <form name="voucher" action="[{$oViewConf->getSelfActionLink()}]" method="post" class="js-oxValidate form-inline" role="form" novalidate="novalidate">
                <div class="couponBox" id="coupon">
                    ...
                    <label class="req basket-coupon-label" for="input_voucherNr">У вас есть промокод?</label>
                    <div class="form-group">
                        <div class="input-group">
                            <input type="text" name="voucherNr" size="30" class="form-control js-oxValidate js-oxValidate_notEmpty basket-coupon-field" id="input_voucherNr" required="required">
                            <span class="input-group-btn">
                                <button type="submit" class="btn btn-primary submitButton"><i class="fa fa-gift"></i>ок</button>
                            </span>
                        </div>
                ...
                [{if false && $oViewConf->getShowVouchers() && $oxcmp_basket->getVoucherDiscValue() }]
                ...
                [{if $oViewConf->getShowVouchers() && $oxcmp_basket->getVoucherDiscValue() }]
                    [{foreach from=$oxcmp_basket->getVouchers() item=sVoucher key=key name=Voucher}]
                        ...
                        <div class="basket-coupon-message">
                            Скидка по промокоду ([{$sVoucher->sVoucherNr}]) [{oxprice price=$sVoucher->dVoucherdiscount*-1 currency=$currency}]
...
<div class="basket-total-sum">[{oxprice price=$oxcmp_basket->getPrice() currency=$currency}]</div>
