    ...
    <tbody>
        ...
        [{foreach key=basketindex from=$oxcmp_basket->getContents() item=basketitem name=basketContents}]
            [{block name="checkout_basketcontents_basketitem"}]
                ...
                <tr id="table_cartItem_[{$smarty.foreach.basketContents.iteration}]" class="table-row basket-item">
                    ...
                    [{block name="checkout_basketcontents_basketitem_quantity"}]
                        ...
                        <td class="quantity">
                            [{if $editable}]
                                ...
                                <input class="basket-item-id" type="hidden" name="aproducts[[{$basketindex}]][basketitemid]" value="[{$basketindex}]">
                                ...
                                [{if !$basketitem->isBundle() || !$basketitem->isDiscountArticle()}]
                                    <div class="quantity-field-wrapper">
                                        <input id="am_[{$smarty.foreach.basketContents.iteration}]" type="text" class="textbox value" name="aproducts[[{$basketindex}]][am]" value="[{$basketitem->getAmount()}]" size="3" min="0"[{if $oConfig->getConfigParam('blAllowUnevenAmounts')}] step="any"[{/if}]>
                    ...
                    [{block name="checkout_basketcontents_basketitem_unitprice"}]
                        ...
                        <td class="unitPrice">
                            ...
                            [{*<div class="price-notice">
                                    При автозаказе
                                    скидка <span class="percent">до 20%</span>
                            </div>*}]
                        </td>
                    [{/block}]
