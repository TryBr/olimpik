[{block name="checkout_steps_main"}]
    <ol class="checkoutSteps checkout-steps-custom">
        [{if $oxcmp_basket->getProductsCount()}]
            [{assign var=showStepLinks value=true}]
        [{/if}]

        <a class="back-link" href="/">&#8592; Вернуться к покупкам</a>

        <li class="step1[{if $active == 1}] active [{elseif $active > 1}] passed [{/if}] [{if $active == 2}] passed-fix [{/if}]">
        ...
        <li class="step2 [{if $active == 2}] active [{elseif $active > 2}] passed passed-fix [{/if}]">
            <a href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=ok_fastorder"}]">
            <div class="num">2</div>
            <div class="text">
                Доставка и оплата
            </div>
        ...
        <li class="step3 [{if $active == 3}] active last-step [{/if}]">
            [{*<a href="#">*}]
            <div class="num">3</div>
            <div class="text">
                Подтверждение
