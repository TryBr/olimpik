...
[{capture append="oxidBlock_pageHead"}]
    [{strip}]
        ...
        <link rel="apple-touch-icon" sizes="180x180" href="[{$oViewConf->getImageUrl('favicons/apple-touch-icon.png')}]">
        <link rel="icon" type="image/png" sizes="32x32" href="[{$oViewConf->getImageUrl('favicons/favicon-32x32.png')}]">
        <link rel="icon" type="image/png" sizes="16x16" href="[{$oViewConf->getImageUrl('favicons/favicon-16x16.png')}]">
        <link rel="manifest" href="[{$oViewConf->getImageUrl('favicons/site.webmanifest')}]">
        <link rel="mask-icon" href="[{$oViewConf->getImageUrl('favicons/safari-pinned-tab.svg')}]" color="#5bbad5">
        ...
        [{/block}]
            ...
            [{oxstyle include="css/custom2.css"}]
...
<html lang="[{$oView->getActiveLangAbbr()}]" [{if $oViewConf->getShowFbConnect()}]xmlns:fb="http://www.facebook.com/2008/fbml"[{/if}]>
    ...
    <body class="sticky-header cl-[{$oView->getClassName()}][{if $smarty.get.plain == '1'}] popup[{/if}][{if $blIsCheckout}] is-checkout[{/if}][{if $oxcmp_user && $oxcmp_user->oxuser__oxpassword->value}] is-logged-in[{/if}]"[{* if $sStyle}] style="[{$sStyle}]"[{/if *}]>
